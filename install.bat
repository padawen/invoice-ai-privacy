...
if errorlevel 1 (
    ...
    powershell -Command "& {Invoke-WebRequest -Uri 'https://www.python.org/ftp/python/3.11.8/python-3.11.8-amd64.exe' -OutFile ([System.IO.Path]::Combine($env:TEMP, 'python-installer.exe'))}"
...
if exist "%USERPROFILE%\AppData\Local\Programs\Ollama\ollama.exe" (
    ...
    powershell -Command "& {Invoke-WebRequest -Uri 'https://ollama.ai/download/OllamaSetup.exe' -OutFile ([System.IO.Path]::Combine($env:TEMP, 'OllamaSetup.exe'))}"
...
if errorlevel 1 (
    ...
    powershell -Command "& {Invoke-WebRequest -Uri 'https://github.com/git-for-windows/git/releases/download/v2.42.0.windows.2/Git-2.42.0.2-64-bit.exe' -OutFile ([System.IO.Path]::Combine($env:TEMP, 'git-installer.exe'))}"
...
if errorlevel 1 (
    ...
    powershell -Command "& {Invoke-WebRequest -Uri 'https://bin.equinox.io/c/bNyj1mQVY4c/ngrok-v3-stable-windows-amd64.zip' -OutFile ([System.IO.Path]::Combine($env:TEMP, 'ngrok.zip'))}"
    powershell -Command "& {Expand-Archive -Path ([System.IO.Path]::Combine($env:TEMP, 'ngrok.zip')) -DestinationPath 'C:\Windows\System32\' -Force}"
    del "%TEMP%\ngrok.zip"
...
echo [MODEL] Downloading Llama 3.1 8B Instruct Q6_K model...
echo This model provides excellent accuracy (~6GB) and may take 10-30 minutes depending on your internet speed.
echo Please be patient...
"%USERPROFILE%\AppData\Local\Programs\Ollama\ollama.exe" pull llama3.1:8b-instruct-q6_K
...
echo - Llama 3.1 8B Instruct Q6_K model (~6GB)
